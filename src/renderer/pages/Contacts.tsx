...
export const ContactsPage: React.FC = () => {
  ...
  const contactService = {
    async getAll(filters: ContactFilters = {}): Promise<ContactModel[]> {
      ...
      if (filters.search) {
        query += ' AND (first_name LIKE ? OR last_name LIKE ? OR email LIKE ? OR phone LIKE ? OR position LIKE ? OR notes LIKE ?)';
      ...
      return result.map((row: any) => ContactModel.fromJSON(row));
    },

    async getById(id: number): Promise<ContactModel | null> {
      const result = await window.electronAPI.queryDatabase('SELECT * FROM contacts WHERE id = ?', [id]);
      return result.length > 0 ? ContactModel.fromJSON(result[0]) : null;
    },
    ...
    async create(data: ContactCreateData): Promise<ContactModel> {
      ...
      const result = await window.electronAPI.executeQuery(
        `INSERT INTO contacts (first_name, last_name, email, phone, position, linkedin_url, notes, company_id, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [data.first_name, data.last_name, data.email, data.phone, data.position, data.linkedin_url, data.notes, data.company_id, now, now]
      );
    ...
    async delete(id: number): Promise<void> {
      await window.electronAPI.executeQuery('DELETE FROM contacts WHERE id = ?', [id]);
    },

    async getStatistics(): Promise<ContactStatistics> {
      const totalResult = await window.electronAPI.queryDatabase('SELECT COUNT(*) as count FROM contacts', []);
      const emailResult = await window.electronAPI.queryDatabase('SELECT COUNT(*) as count FROM contacts WHERE email IS NOT NULL AND email != ""', []);
      const phoneResult = await window.electronAPI.queryDatabase('SELECT COUNT(*) as count FROM contacts WHERE phone IS NOT NULL AND phone != ""', []);
      const linkedinResult = await window.electronAPI.queryDatabase('SELECT COUNT(*) as count FROM contacts WHERE linkedin_url IS NOT NULL AND linkedin_url != ""', []);
      const companyResult = await window.electronAPI.queryDatabase('SELECT COUNT(*) as count FROM contacts WHERE company_id IS NOT NULL', []);
      const recentResult = await window.electronAPI.queryDatabase('SELECT COUNT(*) as count FROM contacts WHERE datetime(created_at) > datetime("now", "-30 days")', []);
      const positionResult = await window.electronAPI.queryDatabase('SELECT position, COUNT(*) as count FROM contacts WHERE position IS NOT NULL AND position != "" GROUP BY position ORDER BY count DESC', []);
      ...
      return {
        ...
        withEmail: emailResult[0]?.count || 0,
        withPhone: phoneResult[0]?.count || 0,
        withLinkedIn: linkedinResult[0]?.count || 0,
        withCompany: companyResult[0]?.count || 0,
    ...
    async findPotentialDuplicates(): Promise<Array<{ contacts: ContactModel[]; reason: string }>> {
      ...
      const emailDuplicates = await window.electronAPI.queryDatabase(
        'SELECT email FROM contacts WHERE email IS NOT NULL AND email != "" GROUP BY email HAVING COUNT(*) > 1',
        []
      );
      
      for (const row of emailDuplicates) {
        const contacts = await window.electronAPI.queryDatabase(
          'SELECT * FROM contacts WHERE email = ?',
          [row.email]
        );
        duplicates.push({
          contacts: contacts.map((c: any) => ContactModel.fromJSON(c)),
          reason: `Gleiche E-Mail-Adresse: ${row.email}`
        });
      }
      
      // Find name duplicates
      const nameDuplicates = await window.electronAPI.queryDatabase(
        'SELECT first_name, last_name FROM contacts WHERE first_name IS NOT NULL AND last_name IS NOT NULL GROUP BY first_name, last_name HAVING COUNT(*) > 1',
        []
      );
      ...
      for (const row of nameDuplicates) {
        ...
        duplicates.push({
          contacts: contacts.map((c: any) => ContactModel.fromJSON(c)),
          reason: `Gleicher Name: ${row.first_name} ${row.last_name}`
        });
      }
    ...
    async exportToCSV(): Promise<string> {
      ...
      const headers = ['ID', 'Vorname', 'Nachname', 'E-Mail', 'Telefon', 'Position', 'LinkedIn', 'Notizen', 'Erstellt', 'Aktualisiert'];
